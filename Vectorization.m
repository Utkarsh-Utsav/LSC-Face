...
NET.addAssembly(fullfile('C:\MccSdk\MccSdk.dll'));

% select 1-3 samples of each user for training data
p = 0;
for i = 1:100
    for j = [1 2 3]
        file1 = strcat(pwd,'\Data\minutiae\', num2str(i), '_', num2str(j),'.iso-fmr');
        p = p + 1;
        file2=strcat(pwd,'\Data\training\', num2str(p),'.ist');
...
K = ones(300,300);
for i = 1:300
    filename1 = strcat(pwd,'\Data\training\', num2str(i), '.ist');
    template1 = BioLab.Biometrics.Mcc.Sdk.MccSdk.CreateMccTemplateFromIsoTemplate(filename1);
    
    for j = 1:i
        filename2 = strcat(pwd,'\Data\training\', num2str(j), '.ist');
...
for i = 1:100
    for j = 4:8
        filename1 = strcat(pwd,'\Data\minutiae\', num2str(i), '_', num2str(j),'.iso-fmr');
        template1 = BioLab.Biometrics.Mcc.Sdk.MccSdk.CreateMccTemplateFromIsoTemplate(filename1);
        
        ft = zeros(1,300);
        for ik = 1:300
            filename2 = strcat(pwd,'\Data\training\', num2str(ik), '.ist');
            template2 = BioLab.Biometrics.Mcc.Sdk.MccSdk.CreateMccTemplateFromIsoTemplate(filename2);
            tmp = BioLab.Biometrics.Mcc.Sdk.MccSdk.MatchMccTemplates(template1, template2);
        ...
        Ftemplate = ft*W;
        file = strcat(pwd,'\Data\data\',num2str(i),'_',num2str(j), '.mat');
        save(file, 'Ftemplate');
    end
end